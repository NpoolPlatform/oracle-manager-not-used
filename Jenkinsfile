  ...
  stages {
    ...
    stage('Config target') {
      ...
      steps {
        ...
        sh (returnStdout: false, script: '''
          ...
          kubectl exec --namespace kube-system mysql-0 -- mysql -h 127.0.0.1 -uroot -p$PASSWORD -P3306 -e "create database if not exists oracle_manager;"
          ...
          for vhost in `cat cmd/*/*.viper.yaml | grep hostname | awk '{print $2}' | sed 's/"//g' | sed 's/\\./-/g'`; do
            ...
            ./apollo-item-config.sh $APP_ID $TARGET_ENV $vhost database_name oracle_manager
            cd -
          done
        '''.stripIndent())
      }
    ...
    stage('Release docker image for development') {
      ...
      steps {
        ...
        sh(returnStdout: false, script: '''
          images=`docker images | grep entropypool | grep oracle-manager | grep none | awk '{ print $3 }'`
    ...
    stage('Release docker image for testing') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          set +e
          docker images | grep oracle-manager | grep $tag
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
    ...
    stage('Deploy for development') {
      ...
      steps {
        sh 'sed -i "s/uhub.service.ucloud.cn/$DOCKER_REGISTRY/g" cmd/oracle-manager/k8s/01-oracle-manager.yaml'
    ...
    stage('Deploy for testing') {
      ...
      steps {
        sh(returnStdout: true, script: '''
          ...
          git reset --hard
          git checkout $tag
          sed -i "s/oracle-manager:latest/oracle-manager:$tag/g" cmd/oracle-manager/k8s/01-oracle-manager.yaml
          sed -i "s/uhub.service.ucloud.cn/$DOCKER_REGISTRY/g" cmd/oracle-manager/k8s/01-oracle-manager.yaml
