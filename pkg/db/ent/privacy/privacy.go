...
// EvalQuery return f(ctx, q).
func (f RewardQueryRuleFunc) EvalQuery(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.RewardQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *ent.RewardQuery", q)
}

// The RewardMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type RewardMutationRuleFunc func(context.Context, *ent.RewardMutation) error

// EvalMutation calls f(ctx, m).
func (f RewardMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.RewardMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *ent.RewardMutation", m)
}
